...
const ChatScreen = ({chatee}) => {
  ...
  const [messages, setMessages] = useState([
    ...
    { type: 'response', text: 'Sure, I can helpasdf asdf asdfa asdf asdsfasdf with that. What seems to be the problem?', date: '2022-01-03' },
  ]);
  ...
  return (
    ...
    <View style={{ flex: 10, flexDirection: 'row', padding: 10 }}>
      ...
      {messages.map((message, index) => (
          <View
            ...
            >
              ...
              <Text style={{  color : message.type === 'response' ? 'black' : 'white',fontSize: 10, marginTop: 5 }}>{message.date}</Text>
            </View>
          </View>
        ))}
          </KeyboardAwareScrollView>
      ...
      <View style={{flex:1,flexDirection: 'row', padding: 10 }}>
        ...
        />
        <TouchableOpacity onPress={onSendMessage} style={{ backgroundColor:"lightgrey", borderColor: "lightgrey", borderWidth:4, borderRadius: 15, marginLeft:5,padding: 3 }}>
          <Text>Send</Text>
        </TouchableOpacity>
      </View>
    </>
